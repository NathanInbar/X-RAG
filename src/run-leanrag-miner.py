...
def _dedup_key(key:str, layer:int) -> str:
    return f"{layer}:{key}"

def _cluster_lookup_key(agg:dict, fallback_layer:int) -> str:
    target_layer = agg.get('layer', fallback_layer)
    dedup_key = agg.get('_dedup_key')
    if dedup_key:
        return dedup_key
    return _dedup_key(agg['key'], target_layer)

async def _aggregate_task(cluster: Cluster, out_aggregates:AsyncList, pbar, layer_idx:int):
    """
    ...
    agg_layer = layer_idx + 1
    dedup_key = _dedup_key(key, agg_layer)

    async with fmap_lock:
        async with acm_lock:
            if dedup_key in findings_map:
                # duplicate aggregate entity generated: merge into original
                findings_map[dedup_key].extend(new_findings)
                agg_clst_map[dedup_key].extend(cluster)
                is_dup = True
            else:
                # first occurrence
                findings_map[dedup_key] = list(new_findings)
                agg_clst_map[dedup_key] = list(cluster)
                is_dup = False

    # only keep one aggregate node per key
    if not is_dup:
        new_parent['layer'] = agg_layer
        new_parent['_dedup_key'] = dedup_key
...
async def aggregate_layer_recursive(layer:int, max_depth:int):
    ...
    _aggregation_tasks = [_aggregate_task(cluster, layer_aggregates, pbar, layer) for cluster in clusters.values()]
    ...
    for agg in layer_aggregates:
        dedup_key = _cluster_lookup_key(agg, layer+1)
        await mg_driver.create_aggregate_entity(agg, agg_clst_map[dedup_key], agg['layer'])
    ...
    for aggJ, aggK in tqdm(combinations(layer_aggregates, 2), desc="aggregate entity inter-relations"):
        ...
        rel_cluster_J = agg_clst_map[_cluster_lookup_key(aggJ, layer+1)]
        rel_cluster_K = agg_clst_map[_cluster_lookup_key(aggK, layer+1)]
        inter_cluster_rel:list[IntrClusterRel] = await mg_driver.get_inter_cluster_relations(rel_cluster_J, rel_cluster_K)
